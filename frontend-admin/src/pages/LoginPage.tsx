...
const LoginPage = () => {
  ...
  const from = (location.state as any)?.from || '/kitchen';
  ...
  // If Auth0 is configured, redirect to Auth0 login immediately
  useEffect(() => {
    if (isAuth0Configured && auth0) {
      ...
      // Don't redirect if still loading, already authenticated, or processing callback

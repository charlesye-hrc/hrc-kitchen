    ...
    const loadAuth0User = async () => {
      ...
      if (auth0IsAuthenticated && auth0User) {
        try {
          ...
          const claimsAdminAccess = auth0User[`${AUTH0_NAMESPACE}/hasAdminAccess`] as boolean | undefined;
          ...
          if (claimsUserId && claimsRoles) {
            ...
            console.log('Auth0 user loaded from claims:', userData, 'hasAdminAccess:', claimsAdminAccess);
            setUser(userData);
            setHasAdminAccess(claimsAdminAccess || false);
            localStorage.setItem('user', JSON.stringify(userData));
            localStorage.setItem('hasAdminAccess', String(claimsAdminAccess || false));
          } else {
            ...
            try {
              ...
              if (backendUser) {
                ...
                console.log('Auth0 user loaded from backend:', userData, 'hasAdminAccess:', backendUser.hasAdminAccess);
                setUser(userData);
                setHasAdminAccess(backendUser.hasAdminAccess || false);
                localStorage.setItem('user', JSON.stringify(userData));
                localStorage.setItem('hasAdminAccess', String(backendUser.hasAdminAccess || false));
              } else {
